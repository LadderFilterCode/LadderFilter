...
template <int bucket_num = 1000, int counter_num = 16, int threshold = 240>
...
public:
    ...
    static constexpr int thres = 15 + threshold;
    ...
    uint64_t *L1 = nullptr; // Layer 1 is organized as word, one word contains 16 counter, one counter consist of 4 bit
    uint16_t *L2 = nullptr; // Layer 2 is organized as counter, one counter consist of 16 bit
...
public:
    StreamClassifier(uint32_t remained, int l1_ratio = 65)
    {
    ...
    uint32_t insert_SC(uint32_t kick_ID, int kick_f)
    {
        int v1 = 1 << 30;
