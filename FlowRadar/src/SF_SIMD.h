...
public:
    ...
    LadderFilter(int _sf_memory1, int _sf_memory2,
                  int _cols, int _key_len, int _counter_len, /* not available in SIMD version, fixed to 8, 16, 8 */
                  int _thres2)
        : bucket_num1(_sf_memory1/ _cols / 3), bucket_num2(_sf_memory2/ _cols / 3),
          thres2(_thres2)
    {
        Bucket1 = new Bucket[(bucket_num1 + 1) / 2];
        Bucket2 = new Bucket[(bucket_num2 + 1) / 2];
        l1Hash = new BOBHash32(750);
        l2Hash = new BOBHash32(800);
    }
